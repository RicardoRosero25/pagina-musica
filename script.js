let audioContext = new (window.AudioContext || window.webkitAudioContext)();
        ...
        function applyLimiter() {
            ...
            const threshold = 0.8; // nivel donde empieza el limitador
            const knee = 0.1;      // zona de transición (soft knee)
            const ratio = 20;      // relación de compresión fuerte (limitador)
            ...
            for (let ch = 0; ch < audioBuffer.numberOfChannels; ch++) {
                ...
                for (let i = 0; i < input.length; i++) {
                    ...
                    if (absX < threshold - knee) {
                        ...
                        output[i] = Math.sign(x) * threshold + (Math.sign(x) * (absX - threshold)) / ratio;
                    } else {
                        const delta = absX - (threshold - knee);
                        const kneeRatio = delta / (2 * knee);
                        const gain = (1 - kneeRatio * (1 - 1 / ratio));
                        output[i] = x * gain;
                    }
                }
            }
